///////////////////////////////////////////////////////////////////////////////
// 圧縮トラックログ → KMLファイル コンバータ Version 1.1
// Copyright(C) 2006-2007 Rinos, All rights reserved.
///////////////////////////////////////////////////////////////////////////////
// gcc -O -Wa gba2kml.cpp -o gba2kml.exe    (mingw32/gcc3.4.2)
...
// KMLフォーマット出力
///////////////////////////////////////////////////////////////////////////////
// http://earth.google.com/kml/kml_tags.htmlを参考にしてみた…
const char TRACKLOGHEAD[] =
	"<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
	"<kml xmlns=\"http://earth.google.com/kml/2.0\">\n"
	"<Document>\n";
const char TRACKLOGTAIL[] = 
	"</Document>\n"
	"</kml>\n";

const char TRACKLOGSEG_OPEN[] = 
	" <Placemark>\n"
	"  <name>%s</name>\n"
	"  <LineString>\n"
	...
	"   <extrude>1</extrude>\n"
	"   <coordinates>\n";
const char TRACKLOGSEG_CLOSE[] = 
	"   </coordinates>\n"
	"  </LineString>\n"
	" </Placemark>\n";

const time_t UNIX2GARMIN = 631033200; // UNIX秒(1970)->GARMIN秒(1990)
const double DMS2KML = 1.0 / 60 / 60 / 1000;

// 出力ファイル。安易だけどとりあえずグローバルに…。
static FILE* gOut = 0;

// GoogleMapsで大量の座標が指定されると非常に重くなるので、軽減のために省略機能を追加してみた。
// 本来は直線近似の誤差に応じて省略ポイントを決めるべきだけど、とりあえず一定間隔で省くだけ。
static u32 gOmit, gOmitCount;
...
bool OpenTrackLog(time_t sec){
	...
	char fname[30];
	sprintf(fname, "%04d%02d%02d-%02d%02d.kml", t->tm_year + 1900, t->tm_mon + 1, t->tm_mday, t->tm_hour, t->tm_min);
	gOut = fopen(fname, "w");
	if(!gOut){
...
bool WriteTrackLog(const s32* v, s32 prc){
	...
	if(gSegFlag){ // セグメント分割フラグ
		if(gSegCount++) fputs(TRACKLOGSEG_CLOSE, gOut);
		struct tm* t = localtime(&sec); // セグメント名にはローカルタイムを使う
		char fname[30];
		sprintf(fname, "#%d:%04d/%02d/%02d %02d:%02d:%02d", gSegCount, t->tm_year + 1900, t->tm_mon + 1, t->tm_mday, t->tm_hour, t->tm_min, t->tm_sec);
		fprintf(gOut, TRACKLOGSEG_OPEN, fname);// ここで直接パラメータを渡しても良いが…
		gSegFlag = 0; // セグメント追加済みマーク
	} else {
		if(gOmitCount++ < gOmit) return true; // 省略
	}
	gOmitCount = 0;

	// 緯度と経度は精度値でシフトして出力
	return fprintf(gOut, "\t%f,%f,%d\n", (v[1] << prc) * DMS2KML, (v[0] << prc) * DMS2KML, v[2]) > 0;
}
...
int main(int argc, char** argv){
	...
	case 2:
		...
		// no break
	default:
		fprintf(stderr, "usage: %s file [segment_option [omit]]\n", *argv);
		return 1;
	}
